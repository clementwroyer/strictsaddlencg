...
K \leq \frac{\Delta f}{c_{\mathcal{R}}}\,\varepsilon_g^{-2} + \frac{\Delta f}{c_{\mathcal{N}}}\,\varepsilon_g^{-\max\{1+p,\, 2(1+p-q)\}} + \frac{3L_2^2\,\Delta f}{2}\,\varepsilon_H^{-3}.
\end{equation}
\end{theorem}

\begin{proof}
Let $K$ be the first index such that the stopping condition holds. Every iteration $k < K$ is either an NCG step ($\|g_k\| \geq \varepsilon_g$) or an NCD step ($\|g_k\| < \varepsilon_g$, $-\lambda_k \geq \varepsilon_H$). Among NCG iterations, each is either restarted or non-restarted.

By Lemma~\ref{lem:decrease_R}, each restarted NCG iteration decreases $f$ by at least $c_{\mathcal{R}}\,\varepsilon_g^2$. By Lemma~\ref{lem:decrease_N}, each non-restarted NCG iteration decreases $f$ by at least $c_{\mathcal{N}}\,\varepsilon_g^{\max\{1+p,\, 2(1+p-q)\}}$. By Lemma~\ref{lem:decrease_NCD}, each NCD iteration decreases $f$ by at least $\frac{2\varepsilon_H^3}{3L_2^2}$.

Telescoping and using A3:
\[
\Delta f \geq K^1_{\mathcal{R}} \cdot c_{\mathcal{R}}\,\varepsilon_g^2 + K^1_{\mathcal{N}} \cdot c_{\mathcal{N}}\,\varepsilon_g^{\max\{1+p,\, 2(1+p-q)\}} + K^2 \cdot \frac{2\varepsilon_H^3}{3L_2^2}.
\]
...
\[
K^1_{\mathcal{R}} \leq \frac{\Delta f^1}{c_{\mathcal{R}}\,\zeta^2}, \qquad K^1_{\mathcal{N}} \leq \frac{\Delta f^1}{c_{\mathcal{N}}\,\zeta^{\max\{1+p,\, 2(1+p-q)\}}}.
\]
Since $\Delta f^1 \leq \Delta f$:
\begin{equation}
\label{eq:regime1}
K^1 = K^1_{\mathcal{R}} + K^1_{\mathcal{N}} \leq \frac{\Delta f}{c_{\mathcal{R}}}\,\zeta^{-2} + \frac{\Delta f}{c_{\mathcal{N}}}\,\zeta^{-\max\{1+p,\, 2(1+p-q)\}}.
\end{equation}
...
Combining Lemma~\ref{lem:decrease_N} with the PL inequality~\eqref{eq:PL}:
\begin{equation}
\label{eq:decrease_N3}
f(x_k) - f(x_{k+1}) \geq c_{\mathcal{N}}\left(2\gamma(f(x_k) - f(x^\star))\right)^r, \qquad r := 1 + p - q.
\end{equation}
Define $C := c_{\mathcal{N}}(2\gamma)^r = \frac{\sigma^2(2\gamma)^r}{2L_1\kappa^2}$. We analyze three cases.

\paragraph{Case 1: $r = 1$ ($p = q$).} The decrease becomes $f(x_k) - f(x_{k+1}) \geq 2\gamma\, c_{\mathcal{N}}(f(x_k) - f(x^\star))$, giving a linear rate identical (up to constants) to the restarted case. Using $\Delta f^3 \leq \frac{L_1}{2}\delta^2$:
\begin{equation}
\label{eq:N3_r1}
K^3_{\mathcal{N},\, p=q} \leq \frac{L_1\kappa^2}{\sigma^2\gamma}\log\frac{L_1\delta^2}{2\varepsilon}.
\end{equation}

\paragraph{Case 2: $r > 1$ ($p > q$, sublinear rate).} We have:
\[
f(x_{k+1}) - f(x^\star) \leq (f(x_k) - f(x^\star))\left(1 - C(f(x_k) - f(x^\star))^{r-1}\right).
\]
Define $\phi_k := (f(x_k) - f(x^\star))^{1-r}$. Raising both sides to the power $1 - r < 0$ flips the inequality. Using $(1-x)^{1-r} \geq 1 + (r-1)x$ for $x \in (0,1)$:
\begin{align*}
\phi_{k+1} &\geq \phi_k(1 - (1-r)C\,\phi_k^{-1}) = \phi_k + (r-1)C.
\end{align*}
Telescoping over $K^3_{\mathcal{N}}$ iterations: $\phi_{K} \geq \phi_0 + K^3_{\mathcal{N}}(r-1)C$. Requiring $f(x_K) - f(x^\star) \leq \varepsilon$ and using $\Delta f^3 \leq \frac{L_1}{2}\delta^2$:
\begin{equation}
\label{eq:N3_r_big}
K^3_{\mathcal{N},\, p>q} \leq \frac{2L_1\kappa^2}{\sigma^2(2\gamma)^r(r-1)}\left(\varepsilon^{1-r} - \left(\frac{L_1\delta^2}{2}\right)^{1-r}\right).
\end{equation}

\paragraph{Case 3: $r < 1$ ($p < q$, finite termination).} Same recursion, but now $1 - r > 0$, so the power preserves the inequality. Using $(1-x)^{1-r} \leq 1 - (1-r)x$ for $x \in (0,1)$ and $1-r \in (0,1)$:
\begin{align*}
\phi_{k+1} &\leq \phi_k(1 - (1-r)C\,\phi_k^{-1}) = \phi_k - (1-r)C.
\end{align*}
Telescoping: $\phi_K \leq \phi_0 - K^3_{\mathcal{N}}(1-r)C$. Since $\phi_K \geq 0$:
\[
K^3_{\mathcal{N}} \leq \frac{\phi_0}{(1-r)C} = \frac{(\Delta f^3)^{1-r}}{(1-r)C}.
\]
At $K^\star = \lfloor \frac{(\Delta f^3)^{1-r}}{(1-r)C} \rfloor$, the recursion forces $\phi_{K^\star} = 0$, i.e., $f(x_{K^\star}) = f(x^\star)$ exactly. Using $\Delta f^3 \leq \frac{L_1}{2}\delta^2$ from~\eqref{eq:entry_gap}:
\begin{equation}
\label{eq:N3_r_small}
K^3_{\mathcal{N},\, p<q} \leq \frac{2L_1\kappa^2}{\sigma^2(2\gamma)^r(1-r)}\left(\frac{L_1\delta^2}{2}\right)^{1-r}.
\end{equation}
This bound is independent of $\varepsilon$: the algorithm reaches the exact minimizer in finitely many non-restarted steps.
...
\label{eq:total}
\boxed{
K \leq \underbrace{\frac{\Delta f}{c_{\mathcal{R}}}\,\zeta^{-2} + \frac{\Delta f}{c_{\mathcal{N}}}\,\zeta^{-\max\{1+p,\, 2(1+p-q)\}}}_{\text{Regime 1}} + \underbrace{\frac{3L_2^2\,\Delta f}{2\beta^3}}_{\text{Regime 2}} + \underbrace{\frac{L_1}{\gamma}\log\frac{L_1\delta^2}{2\varepsilon} + K^3_{\mathcal{N}}(r)}_{\text{Regime 3}}
}
\end{equation}

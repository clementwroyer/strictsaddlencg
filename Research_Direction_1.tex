...
All iterations draw from the single decrease budget $\Delta_0$:
\begin{equation}
\label{eq:budget}
\begin{split}
...
\subsubsection{Non-restarted iterations in Regime 3 ($k \in \mathcal{N}^3$) Using Potential Function Proof}
...
\subsubsection{Non-restarted iterations in Regime 3 ($k \in \mathcal{N}^3$) With (1-C) Proof}

\paragraph{Step 1: Per-iteration decrease via descent lemma.}
By the descent lemma~\eqref{eq:descent_lemma}, using the non-restarted 
conditions $g_k^Td_k \leq -\sigma\|g_k\|^{1+p}$ and $\|d_k\| \leq \kappa\|g_k\|^q$:
\[
f(x_k - \alpha g_k) \leq f(x_k) - \left(\alpha - \frac{L_1\alpha^2}{2}\right)\|g_k\|^2.
\]
Applying the PL inequality~\eqref{eq:PL}, $\|g_k\|^2 \geq 2\gamma(f(x_k) - f(x^\star))$:
\[
f(x_k - \alpha g_k) \leq f(x_k) - \left(\alpha - \frac{L_1\alpha^2}{2}\right) 
2\gamma(f(x_k) - f(x^\star)).
\]
Hence, letting $\Delta_k := f(x_k) - f(x^\star)$:
\begin{equation}
\label{eq:descent_R3_N}
\Delta_{k+1} \leq \left(1 - 2\gamma\left(\alpha - \frac{L_1\alpha^2}{2}\right)\right)\Delta_k.
\end{equation}
The contraction factor $C = 2\gamma\!\left(\alpha - \frac{L_1\alpha^2}{2}\right)$ 
depends on the stepsize $\alpha$. We now compute it for restarted 
and non-restarted stepsizes separately.

\paragraph{Step 2: Computing $C_{\mathcal{N}}$ for non-restarted iterations.}
Substituting the optimal non-restarted stepsize 
$\alpha^\star_{\mathcal{N}} = \frac{\sigma}{L_1\kappa^2}\|g_k\|^{1+p-2q}$:
\begin{align*}
2\gamma\!\left(\alpha^\star_{\mathcal{N}} - \frac{L_1(\alpha^\star_{\mathcal{N}})^2}{2}\right)
&= 2\gamma\!\left(\frac{\sigma}{L_1\kappa^2}\|g_k\|^{1+p-2q} 
   - \frac{L_1}{2}\cdot\frac{\sigma^2}{L_1^2\kappa^4}\|g_k\|^{2(1+p-2q)}\right) \\
&= 2\gamma\left(\frac{\sigma}{L_1\kappa^2}\|g_k\|^{1+p-2q} 
   - \frac{\sigma^2}{2L_1\kappa^4}\|g_k\|^{2(1+p-2q)}\right) \\
&= 2\gamma\|g_k\|^{1+p-2q}\left(\frac{\sigma}{L_1\kappa^2} 
   - \frac{\sigma^2}{2L_1\kappa^4}\|g_k\|^{1+p-2q}\right) \\
&= \frac{2\gamma\sigma}{L_1\kappa^2}\|g_k\|^{s}
   \left(1 - \frac{\sigma}{2\kappa^2}\|g_k\|^{s}\right),
\end{align*}
where $s := 1+p-2q$. We define:
\begin{equation}
\label{eq:C_N}
C_{\mathcal{N},k} := \frac{2\gamma\sigma}{L_1\kappa^2}\|g_k\|^{s}
\left(1 - \frac{\sigma}{2\kappa^2}\|g_k\|^{s}\right).
\end{equation}

\paragraph{Step 3: Analysis of $C_{\mathcal{N},k}$ as a function of $\|g_k\|$.}
Let $u := \|g_k\|^s$, so that $C_{\mathcal{N},k} = \frac{2\gamma\sigma}{L_1\kappa^2}h(u)$ where:
\[
h(u) := u - \frac{\sigma}{2\kappa^2}u^2.
\]
This is a concave quadratic in $u$, opening downward. Taking the derivative:
\[
\frac{\partial h}{\partial u} = 1 - \frac{\sigma}{\kappa^2}u = 0 
\implies u^\star = \frac{\kappa^2}{\sigma}.
\]
Hence $h$ attains its maximum at $u^\star = \frac{\kappa^2}{\sigma}$. 
Moreover, $h(u) > 0$ if and only if:
\[
u\!\left(1 - \frac{\sigma}{2\kappa^2}u\right) > 0.
\]
Since $u > 0$, this reduces to $u < \frac{2\kappa^2}{\sigma}$, 
so $h(u) > 0$ on $\left(0, \frac{2\kappa^2}{\sigma}\right)$.

\paragraph{Step 4: Lower bound on $C_{\mathcal{N},k}$.}
In Regime 3, two bounds on $\|g_k\|$ hold simultaneously:
\begin{itemize}
    \item \textbf{Upper bound:} $\|g_k\| \leq L_1\delta$ 
    (from~\eqref{eq:entry_grad_upper}).
    \item \textbf{Lower bound:} $\|g_k\| \geq \varepsilon_g$ 
    (algorithm has not yet converged).
\end{itemize}
Hence $u = \|g_k\|^s \in \left[\varepsilon_g^s,\, (L_1\delta)^s\right]$ if $s > 0$, 
and $u \in \left[(L_1\delta)^s,\, \varepsilon_g^s\right]$ if $s < 0$. 
Since $h$ is increasing on $(0, u^\star)$, the minimum of $C_{\mathcal{N},k}$ 
depends on the sign of $s$:

\begin{itemize}
    \item \textbf{Case 1: $s = 0$ ($q = \frac{1+p}{2}$).} 
    $u = 1$ is constant, so $C_{\mathcal{N},k}$ is already independent of $k$:
    \[
    C_{\min} = \frac{2\gamma\sigma}{L_1\kappa^2}\!\left(1 - \frac{\sigma}{2\kappa^2}\right).
    \]

    \item \textbf{Case 2: $s > 0$ ($q < \frac{1+p}{2}$).} 
    $u$ is increasing in $\|g_k\|$, so its minimum is at $\|g_k\| = \varepsilon_g$:
    \[
    C_{\min} = \frac{2\gamma\sigma}{L_1\kappa^2}\,\varepsilon_g^s
    \!\left(1 - \frac{\sigma}{2\kappa^2}\varepsilon_g^s\right).
    \]

    \item \textbf{Case 3: $s < 0$ ($q > \frac{1+p}{2}$).} 
    $u$ is decreasing in $\|g_k\|$, so its minimum is at $\|g_k\| = L_1\delta$:
    \[
    C_{\min} = \frac{2\gamma\sigma}{L_1\kappa^2}(L_1\delta)^s
    \!\left(1 - \frac{\sigma}{2\kappa^2}(L_1\delta)^s\right).
    \]
\end{itemize}

In all cases, $C_{\mathcal{N},k} \geq C_{\min} > 0$, 
and~\eqref{eq:descent_R3_N} gives the uniform geometric contraction:
\begin{equation}
\label{eq:geometric_N3}
\Delta_{k+1} \leq (1 - C_{\min})\,\Delta_k.
\end{equation}

\paragraph{Step 5: Complexity bound.}
Iterating~\eqref{eq:geometric_N3} over $K$ non-restarted iterations:
\[
\Delta_K \leq (1 - C_{\min})^K \Delta_0 
\leq (1 - C_{\min})^K \cdot \frac{L_1\delta^2}{2}.
\]
Requiring $\Delta_K \leq \varepsilon_g^2/(2L_1)$ and 
using $-\log(1-C) \geq C$ for $C \in (0,1)$:
\[
K \leq \frac{1}{C_{\min}}\log\frac{L_1^2\delta^2}{\varepsilon_g^2}.
\]
Substituting $C_{\min}$ in each case:
\begin{equation}
\label{eq:K_N3}
K^3_{\mathcal{N}} \leq 
\begin{cases}
\dfrac{L_1\kappa^2}{2\gamma\sigma\!\left(1 - \frac{\sigma}{2\kappa^2}\right)}
\log\dfrac{L_1^2\delta^2}{\varepsilon_g^2} 
& \text{if } s = 0, \\[14pt]
\dfrac{L_1\kappa^2}{2\gamma\sigma\,\varepsilon_g^s\!\left(1 
- \frac{\sigma}{2\kappa^2}\varepsilon_g^s\right)}
\log\dfrac{L_1^2\delta^2}{\varepsilon_g^2} 
& \text{if } s > 0, \\[14pt]
\dfrac{L_1\kappa^2}{2\gamma\sigma\,(L_1\delta)^s\!\left(1 
- \frac{\sigma}{2\kappa^2}(L_1\delta)^s\right)}
\log\dfrac{L_1^2\delta^2}{\varepsilon_g^2} 
& \text{if } s < 0,
\end{cases}
\end{equation}
...
This proof technique gives linear convergence for $s = 0$ and $s < 0$ and sublinear convergence for $s > 0$.

\subsubsection{Proofs Comparison}

Comparison of the two proof techniques is hard since they use different thresholds for different cases. 
\paragraph{Potential Function proof.} In this proof, we compare $r = 1+p-q$ to 1. This amounts to comparing if $p = q$, 
$p < q$, or $p > q$.

\paragraph{Geometric Recursion Proof.} In this proof, we compare $s = 1+p-2q$ to 0. 
This amounts to comparing if $q = \frac{1+p}{2}$, $q < \frac{1+p}{2}$, or $q > \frac{1+p}{2}$.

Therefore, the results are not directly comparable. However, we can still say that the potential function proof
gives 3 possible convergence regimes: linear, sublinear, and finite termination whereas the geometric recursion proof
gives sublinear and linear convergences.\\
The two approaches carve the $(p, q)$ parameter space differently.
For example, let's take $p = 0.5$ and $q = 0.6$.\\
$r = 1 + 0.5 - 0.6 = 0.9 < 1 \rightarrow$ Finite termination with potential function proof.\\
$s = 1 + 0.5 - 1.2 = 0.3 > 0 \rightarrow$ Sublinear rate with geometric recursion proof.

\noindent Another example would be $p = 0.5$ and $q = 0.8$.\\
$r = 1 + 0.5 - 0.8 = 0.7 < 1 \rightarrow$ Finite termination with potential function proof.\\
$s = 1 + 0.5 - 1.6 = -0.1 < 0 \rightarrow$ Linear rate with geometric recursion proof. 

The following graph shows the space partition made by each proof:
...
%% Bottom region: below both lines
%% For p in [0,1]: below q=p; for p in [1,2]: below q=(1+p)/2
\addplot[fill=colorBlue, opacity=0.6, draw=none] 
    coordinates {(0,0) (1,1) (2,1.5) (2,0) (0,0)};

%% Middle-left region: between q=p (below) and q=(1+p)/2 (above), for p in [0,1]
\addplot[fill=colorMiddle, opacity=0.7, draw=none]
    coordinates {(0,0) (1,1) (0,0.5) (0,0)};

%% Middle-right region: between q=(1+p)/2 (below) and q=p (above), for p in [1,2]
\addplot[fill=colorMiddle, opacity=0.7, draw=none]
    coordinates {(1,1) (2,2) (2,1.5) (1,1)};

%% Top region: above both lines
\addplot[fill=colorR1finite, opacity=0.7, draw=none]
    coordinates {(0,0.5) (1,1) (2,2) (2,2) (0,2) (0,0.5)};

%% --- Threshold lines ---

%% Route 1: q = p
\addplot[color=lineRoute1, thick, -stealth, samples=2, domain=0:2]
    {x};
\node[color=lineRoute1, font=\small, rotate=45, above] at (axis cs:1.6,1.6) {$q = p$};

%% Route 2: q = (1+p)/2
\addplot[color=lineRoute2, thick, dashed, -stealth, samples=2, domain=0:2]
    {(1+x)/2};
\node[color=lineRoute2, font=\small, rotate=26, below] at (axis cs:1.5,1.1) 
    {$q = \frac{1+p}{2}$};

%% Intersection point (1,1)
\addplot[mark=*, mark size=2.5pt, mark options={fill=white, draw=black, line width=1pt}]
    coordinates {(1,1)};
\node[font=\tiny, above right] at (axis cs:1,1) {$(1,1)$};

%% --- Region labels ---

%% Top region
\node[font=\tiny, align=center, color=lineRoute2!70!black] at (axis cs:0.4,1.6)
    {P1: finite term.\\ P2: linear conv.};

%% Middle-left region
\node[font=\tiny, align=center, color=teal!70!black] at (axis cs:0.25,0.6)
    {P1: finite term.\\ P2: sublinear};

%% Middle-right region
\node[font=\tiny, align=center, color=teal!70!black] at (axis cs:1.7,1.75)
    {P1: sublinear\\ P2: linear conv.};

%% Bottom region
\node[font=\tiny, align=center, color=lineRoute1!70!black] at (axis cs:1.5,0.4)
    {P1: sublinear\\ P2: sublinear};

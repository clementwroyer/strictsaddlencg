...
\definecolor{colorR1finite}{RGB}{250,236,231}
\definecolor{colorMiddle}{RGB}{225,245,238}
\definecolor{colorBlue}{RGB}{230,241,251}
\definecolor{lineRoute1}{RGB}{55,138,221}
\definecolor{lineRoute2}{RGB}{216,90,48}
...
for an arbitrary non-convex function. We study this setting under the hypothesis of known Lipschitz constants. 
Therefore, no line search is performed in this case. This case is trivial as the 
complexity of the algorithm is simply the sum of the complexities found in the two papers mentioned:
\begin{equation}
    K = O(\epsilon_g^{-2} + \epsilon_g^{-2(1+p-q)} + \epsilon_H^{-3})
\end{equation}

Subsequently, we analyze the hybrid framework under the hypothesis that the non-convex function 
follows the strict saddle property. This setting gives us three regimes to analyze based on the strit saddle conditions:
\begin{itemize}
    \item \textbf{Regime 1}: High gradient. In this case, we apply the standard NCG algorithm considered 
    in~\cite{RoyerChan2022}. We get the same complexity (up to constants):
    \begin{equation}
        K = O(\alpha^{-2} + \alpha^{-2(1 + p - q)})
    \end{equation}
    \item \textbf{Regime 2}: Negative Curvature. In this case, we apply the standard NCD algorithm 
    considered in~\cite{WrightMa2022}. We get the same complexity:
    \begin{equation}
        K = O(\beta^{-3})
    \end{equation}
    \item \textbf{Regime 3}: Local strong convexity. The complexity of this regime is partitioned into 3 cases based 
    on values of $p$ and $q$:
        \begin{itemize}
...
In Section~\ref{sec:regime3stay}, we first prove that once we enter regime~3, we stay in the strong convexity
region $B(x^\star, 2\delta)$. We also show that it is not possible to prove that once we enter Regime~3, we stay in 
regime~3 from the first iteration done in this regime ($B(x^\star, \delta)$). However, we find specific values for the number of iterations done in regime~3 after which
we stay in regime~3 for both the restarted and non-restarted cases.
...
We write $g_k = \nabla f(x_k)$ for the gradient at iterate $x_k$, $x^\star$ the minimizer of $f$ and 
$f(x^\star)$ the minimum value of $f$.
...
\begin{description}
    ...
    \item[A3.] $f$ is bounded below: there exists $f(x^\star) \in \mathbb{R}$ such that $f(x) \geq f(x^\star)$ for all $x \in \mathbb{R}^n$.
\end{description}

We denote the gap to the minimum by $\Delta_i := f(x_i) - f(x^\star)$.
...
\begin{equation}
\label{eq:stepsize}
\alpha_{\mathcal{N},k}^\star = \frac{\sigma}{L_1\kappa^2}\|g_k\|^{1+p-2q}
\end{equation}
...
K \leq \frac{\Delta_0}{c_{\mathcal{R}}}\,\varepsilon_g^{-2} + \frac{\Delta_0}{c_{\mathcal{N}}}\,\varepsilon_g^{-2(1+p-q)} + \frac{3L_2^2\,\Delta_0}{2}\,\varepsilon_H^{-3}.
\end{equation}
\end{theorem}
...
\[
\Delta_0 \geq K^1_{\mathcal{R}} \cdot c_{\mathcal{R}}\,\varepsilon_g^2 + K^1_{\mathcal{N}} \cdot c_{\mathcal{N}}\,\varepsilon_g^{2(1+p-q)} + K^2 \cdot \frac{2\varepsilon_H^3}{3L_2^2}.
\]
Since all terms are positive, each is individually bounded by $\Delta_0$. Summing the resulting bounds on $K^1_{\mathcal{R}}$, $K^1_{\mathcal{N}}$, and $K^2$ gives~\eqref{eq:bound_arbitrary}.
\end{proof}

\begin{remark}
The bound~\eqref{eq:bound_arbitrary} is conservative because each group uses the full budget $\Delta_0$ independently. The NCG terms match those of Theorem~3.1 in~\cite{RoyerChan2022} (up to constants due to the absence of line search), while the NCD term follows from~\cite{WrightMa2022}.
\end{remark}

% ---------------------------------------------------------------
%  Section 5 — Complexity on Strict Saddle Functions
% ---------------------------------------------------------------

\section{Complexity on Strict Saddle Functions}
\label{sec:strict_saddle}

We now specialize to functions satisfying the strict saddle property,
which provides additional structure near stationary points and allows a
finer complexity analysis than the generic nonconvex setting of
...
$(\zeta,\beta,\gamma,\delta)$-\emph{strict saddle} if, for every
...
\begin{enumerate}
  ...
  \item \textbf{(Near a local minimizer)}\quad there exists a local
        minimizer $x^\star$ with $\|x-x^\star\|\leq\delta$, and
        $f\!\restriction_{B(x^\star,2\delta)}$ is $\gamma$-strongly convex.
\end{enumerate}
\end{definition}

Setting $\varepsilon_g\leq\zeta$ and $\varepsilon_H\leq\beta$ in
Algorithm~\ref{algo:hybrid} ensures that, upon termination,
condition~(3) holds: the output lies near a local minimizer. The three
conditions define three mutually exclusive \emph{regimes} at each
iterate, which we analyze separately.

\paragraph{Notation.}
Let $K^j_{\mathcal{R}}$ and $K^j_{\mathcal{N}}$ denote the number
of restarted and non-restarted NCG iterations in Regime~$j$
respectively, $K^2$ the number of negative curvature iterations,
and $K = K^1_{\mathcal{R}} + K^1_{\mathcal{N}} + K^2 +
K^3_{\mathcal{R}} + K^3_{\mathcal{N}}$ the total iteration count.
We write $\Delta_k := f(x_k) - f(x^\star)$ for the suboptimality
at iterate $k$, where $x^\star$ denotes the local minimizer
associated with the current regime. Since all per-iteration
decreases are non negative and telescope to at most $\Delta_0$, each
regime's iteration count is bounded by dividing $\Delta_0$ by the
minimum per-iteration decrease in that regime.

% -------------------------------------------------------------------
\subsection{Regime 1: Large gradient ($\|g_k\|\geq\zeta$)}
% -------------------------------------------------------------------

In Regime~1, every iterate satisfies $\|g_k\|\geq\zeta$.
Lemmas~\ref{lem:decrease_R} and~\ref{lem:decrease_N} give minimum
per-iteration decreases of $c_{\mathcal{R}}\zeta^2$ and
$c_{\mathcal{N}}\zeta^{2(1+p-q)}$ for restarted and non-restarted
iterations respectively. Telescoping against $\Delta_0$:
\begin{equation}
\label{eq:regime1}
K^1 \;=\; K^1_{\mathcal{R}}+K^1_{\mathcal{N}}
\;\leq\;
\frac{\Delta_0}{c_{\mathcal{R}}}\,\zeta^{-2}
\;+\;
\frac{\Delta_0}{c_{\mathcal{N}}}\,\zeta^{-2(1+p-q)}.
\end{equation}

% -------------------------------------------------------------------
\subsection{Regime 2: Negative curvature
            ($\lambda_{\min}(\nabla^2 f(x_k))\leq-\beta$)}
% -------------------------------------------------------------------

In Regime~2, every iterate satisfies
$\lambda_{\min}(\nabla^2 f(x_k))\leq-\beta$.
Lemma~\ref{lem:decrease_NCD} guarantees a minimum per-iteration
decrease of $2\beta^3/(3L_2^2)$. Telescoping against $\Delta_0$:
\begin{equation}
\label{eq:regime2}
K^2 \;\leq\; \frac{3L_2^2\,\Delta_0}{2\beta^3}.
\end{equation}
...
Throughout this section we assume $x_k\in B(x^\star,\delta)$ and
that $f$ is $\gamma$-strongly convex in $B(x^\star,2\delta)$. Both
the smoothness upper bound~\eqref{eq:smooth_upper} and the PL
inequality~\eqref{eq:PL} are available. We write
$\Delta_k:=f(x_k)-f(x^\star)$ for the suboptimality at iterate $k$.

The gradient stopping criterion $\|g_k\|\leq\varepsilon_g$ is
equivalent to requiring $\Delta_k\leq\varepsilon$ via the smoothness
bound~\eqref{eq:smooth_upper}, where we set:
\begin{equation}
...
% -------------------------------------------------------------------
\subsubsection{Entry conditions and the role of $\delta$}
\label{subsubsec:entry}
% -------------------------------------------------------------------

At the first Regime-3 iterate $k_0$, the strict saddle property
gives $\|x_{k_0}-x^\star\|\leq\delta$. Applying the descent lemma
from $x^\star$ with $\nabla f(x^\star)=0$:
\begin{equation}
\label{eq:entry_upper}
\Delta_{k_0} \;\leq\; \tfrac{L_1}{2}\delta^2.
\end{equation}
Strong convexity at $x_{k_0}$ gives the matching lower bound:
\begin{equation}
\label{eq:entry_lower}
\Delta_{k_0} \;\geq\; \tfrac{\gamma}{2}\|x_{k_0}-x^\star\|^2.
\end{equation}
Combining with~\eqref{eq:smooth_upper} and~\eqref{eq:PL}:
\begin{equation}
\label{eq:entry_grad}
\gamma\|x_{k_0}-x^\star\| \;\leq\; \|g_{k_0}\| \;\leq\; L_1\delta.
\end{equation}

% -------------------------------------------------------------------
\subsubsection{Restarted iterations in Regime 3
               ($k\in\mathcal{R}^3$)}
% -------------------------------------------------------------------

For restarted iterations $d_k=-g_k$ with stepsize $\alpha=1/L_1$,
the descent lemma and PL inequality give the standard gradient-descent
contraction:
\begin{equation}
\label{eq:R3_decrease}
\Delta_{k+1} \;\leq\; \Bigl(1-\frac{\gamma}{L_1}\Bigr)\Delta_k.
\end{equation}
Telescoping and requiring $\Delta_K\leq\varepsilon_g^2/(2L_1)$:
\begin{equation}
\label{eq:K_R3}
K^3_{\mathcal{R}} \;\leq\; \frac{L_1}{\gamma}
\log\frac{L_1^2\delta^2}{\varepsilon_g^2}.
\end{equation}

% -------------------------------------------------------------------
\subsubsection{Non-restarted iterations in Regime 3
               ($k\in\mathcal{N}^3$)}
\label{subsubsec:N3}
% -------------------------------------------------------------------

We present two complexity results for non-restarted iterations.
Theorem~\ref{thm:lyapunov} uses a Lyapunov function argument and
gives the sharpest possible convergence classification, including
finite termination. Theorem~\ref{thm:geometric} uses a uniform
geometric contraction and produces a bound directly comparable to
\cite{RoyerChan2022}.\\
Lemma~\ref{lem:decrease_N} gives, for every $k\in\mathcal{N}^3$:
\begin{equation}
\label{eq:base_decrease}
\Delta_k - \Delta_{k+1} \;\geq\; c_{\mathcal{N}}\|g_k\|^{2r},
\qquad r:=1+p-q,
\end{equation}
where $c_{\mathcal{N}}=\sigma^2/(2L_1\kappa^2)$.
Applying the PL inequality $\|g_k\|^2\geq 2\gamma\Delta_k$
to~\eqref{eq:base_decrease} yields the fundamental recurrence:
\begin{equation}
\label{eq:Delta_recurrence}
\Delta_{k+1} \;\leq\; \Delta_k - C\Delta_k^r,
\qquad C \;:=\; c_{\mathcal{N}}(2\gamma)^r
= \frac{\sigma^2(2\gamma)^r}{2L_1\kappa^2}.
\end{equation}
Both theorems below depart from~\eqref{eq:Delta_recurrence}.

% ===================================================================
\begin{theorem}[Lyapunov function bound]
\label{thm:lyapunov}
Let $f$ satisfy A1--A2 and the $(\zeta,\beta,\gamma,\delta)$-strict
saddle property. Let $\{x_k\}_{k\in\mathcal{N}^3}$ be generated by
Algorithm~\ref{algo:hybrid} with stepsize
$\alpha^\star=\tfrac{\sigma}{L_1\kappa^2}\|g_k\|^{1+p-2q}$, and let
$r:=1+p-q$ and $C:=\tfrac{\sigma^2(2\gamma)^r}{2L_1\kappa^2}$.
\begin{enumerate}[label=(\roman*)]
  \item \textbf{Linear convergence} ($p=q$, $r=1$):
  \begin{equation}
  \label{eq:N3_r1}
  K^3_{\mathcal{N},\,p=q} \;\leq\;
  \frac{L_1\kappa^2}{\sigma^2\gamma}
  \log\frac{L_1^2\delta^2}{\varepsilon_g^2}.
  \end{equation}

  \item \textbf{Sublinear convergence} ($p>q$, $r>1$):
  \begin{equation}
  \label{eq:N3_r_big}
  K^3_{\mathcal{N},\,p>q} \;\leq\;
  \frac{2L_1\kappa^2}{\sigma^2(2\gamma)^r(p-q)}
  \left[
    \left(\frac{\varepsilon_g^2}{2L_1}\right)^{q-p}
    -\left(\frac{L_1\delta^2}{2}\right)^{q-p}
  \right].
  \end{equation}

  \item \textbf{Finite termination} ($p<q$, $r<1$):
  \begin{equation}
  \label{eq:N3_r_small}
  K^3_{\mathcal{N},\,p<q} \;\leq\;
  \left(\frac{L_1\delta^2}{2}\right)^{q-p}
  \frac{2L_1\kappa^2}{\sigma^2(2\gamma)^r(q-p)}.
  \end{equation}
  This bound is independent of $\varepsilon_g$: the algorithm
  reaches $f(x_{K^\star})=f(x^\star)$ exactly in finitely many
  non-restarted steps.
\end{enumerate}
\end{theorem}

\begin{proof}
All three cases depart from recurrence~\eqref{eq:Delta_recurrence}.

\medskip
\noindent\textit{Case (i): $r=1$.}
Recurrence~\eqref{eq:Delta_recurrence} becomes
$\Delta_{k+1}\leq(1-C)\Delta_k$, a geometric contraction with
$C=\sigma^2\gamma/(L_1\kappa^2)\in(0,1)$.
Telescoping over $K^3_{\mathcal{N}}$ iterations and using
$-\log(1-C)\geq C$:
\[
K^3_{\mathcal{N}} \;\leq\; \frac{1}{C}\log\frac{\Delta_{k_0}}{\varepsilon}
\;\leq\; \frac{L_1\kappa^2}{\sigma^2\gamma}\log\frac{L_1^2\delta^2}{\varepsilon_g^2},
\]
where we substituted~\eqref{eq:entry_upper} and
$\varepsilon=\varepsilon_g^2/(2L_1)$.

\medskip
\noindent\textit{Case (ii): $r>1$.}
Define the Lyapunov function $\phi_k:=\Delta_k^{1-r}$, where $1-r<0$.
Since $t\mapsto t^{1-r}$ is \emph{convex} for $r>1$, raising both
sides of~\eqref{eq:Delta_recurrence} to the power $1-r<0$ flips the
inequality:
\[
\phi_{k+1} \;\geq\; \Delta_k^{1-r}(1-C\Delta_k^{r-1})^{1-r}
\;=\; \phi_k(1-C\phi_k^{-1})^{1-r}.
\]
Applying the convexity bound $(1-x)^{1-r}\geq 1+(r-1)x$ for
$x\in(0,1)$ (which holds since $t\mapsto t^{1-r}$ is convex):
\[
\phi_{k+1} \;\geq\; \phi_k\bigl(1+(r-1)C\phi_k^{-1}\bigr)
= \phi_k + (r-1)C.
\]
We first verify that $a := C\Delta_k^{r-1} \in (0,1)$.
Positivity is immediate since $C > 0$ and $\Delta_k > 0$.
For the upper bound, suppose $a \geq 1$: then the right-hand
side of~\eqref{eq:Delta_recurrence} satisfies
$\Delta_k - C\Delta_k^r = \Delta_k(1 - a) \leq 0$,
giving $\Delta_{k+1} \leq f(x^\star)$. Since $x_{k+1}
\in B(x^\star, 2\delta)$ where $f$ is strongly convex,
$f(x_{k+1}) \geq f(x^\star)$, so $\Delta_{k+1} = 0$:
the algorithm has already converged. Hence for any
non-terminated run, $a \in (0,1)$.
Telescoping over $K^3_{\mathcal{N}}$ iterations:
\[
\phi_K \;\geq\; \phi_0 + K^3_{\mathcal{N}}(r-1)C.
\]
Requiring $\Delta_K\leq\varepsilon$ with $1-r<0$ gives
$\phi_K\geq\varepsilon^{1-r}$, hence:
\[
K^3_{\mathcal{N}} \;\leq\;
\frac{\varepsilon^{1-r}-\phi_0}{(r-1)C}.
\]
Substituting $\phi_0=\Delta_{k_0}^{1-r}\geq(L_1\delta^2/2)^{1-r}$,
$C=\sigma^2(2\gamma)^r/(2L_1\kappa^2)$, and
$\varepsilon=\varepsilon_g^2/(2L_1)$ yields~\eqref{eq:N3_r_big}.

\medskip
\noindent\textit{Case (iii): $r<1$, $r > 0$.}
Define the Lyapunov function $\phi_k:=\Delta_k^{1-r}$, where
$1-r>0$. The function $t\mapsto t^{1-r}$ is \emph{concave} for
$0<r<1$. Raising both sides of~\eqref{eq:Delta_recurrence} to the
power $1-r>0$ preserves the inequality:
\[
\phi_{k+1} \;\leq\; \phi_k(1-C\phi_k^{-1})^{1-r}.
\]
Applying the concavity bound $(1-x)^{1-r}\leq 1+(r-1)x$ for
$x\in(0,1)$:
\[
\phi_{k+1} \;\leq\; \phi_k\bigl(1+(r-1)C\phi_k^{-1}\bigr)
= \phi_k - (1-r)C.
\]
Hence $\phi_k$ decreases by the fixed amount $(1-r)C>0$ at every
...
\[
K^3_{\mathcal{N}} \;\leq\; \frac{\phi_0}{(1-r)C}.
\]
At $K^\star=\lceil\phi_0/((1-r)C)\rceil$, the recursion requires
$\phi_{K^\star}\leq 0$. Since $\phi_{K^\star}\geq 0$, the only
consistent value is $\phi_{K^\star}=0$, i.e.\
$f(x_{K^\star})=f(x^\star)$ exactly.
Substituting $\phi_0\leq(L_1\delta^2/2)^{1-r}$ and
$C=\sigma^2(2\gamma)^r/(2L_1\kappa^2)$
yields~\eqref{eq:N3_r_small}.
\end{proof}

\begin{remark}[Extension to $r\leq 0$]
\label{rem:r_negative}
When $q>1+p$, i.e.\ $r\leq 0$, the concavity argument above no
longer applies since $t\mapsto t^{1-r}$ becomes convex. However,
since $r\leq 0$ and $\Delta_k$ is decreasing, $\Delta_k^r$ is
increasing, so $\Delta_k^r\geq\Delta_{k_0}^r$ for all $k\geq k_0$.
Recurrence~\eqref{eq:Delta_recurrence} then gives
$\Delta_{k+1}\leq\Delta_k - C\Delta_{k_0}^r$: a constant decrease
of $C\Delta_{k_0}^r$ per step. The algorithm therefore reaches
$f(x^\star)$ exactly in at most
$\lceil\Delta_{k_0}/(C\Delta_{k_0}^r)\rceil
=\lceil\Delta_{k_0}^{1-r}/C\rceil$
iterations. It yields finite termination with a different constant than
\eqref{eq:N3_r_small}, but the same qualitative conclusion.
\end{remark}
% ===================================================================

\begin{remark}[Comparison with gradient descent]
\label{rem:gd_comparison}
Bound~\eqref{eq:N3_r1} matches the restarted rate~\eqref{eq:K_R3}
up to the factor $\sigma^2/\kappa^2\leq 1$, recovering the gradient
descent baseline for $p=q$. Bound~\eqref{eq:N3_r_small} establishes
that, for any $p<q$, the non-restarted NCG phase reaches the
\emph{exact} minimizer in finitely many steps which an
improvement over gradient descent, which can only guarantee
$\Delta_k\leq\varepsilon$ for any fixed $\varepsilon>0$.
\end{remark}
...
Under A1--A3, suppose $f$ is $(\zeta,\beta,\gamma,\delta)$-strict
saddle. Then Algorithm~\ref{algo:hybrid} with $\varepsilon_g=\zeta$
and $\varepsilon_H=\beta$ reaches a point near a local minimizer in
at most $K$ iterations, where:
\begin{equation}
\label{eq:total}
K \;\leq\;
\underbrace{\frac{\Delta_0}{c_{\mathcal{R}}}\,\zeta^{-2}
+ \frac{\Delta_0}{c_{\mathcal{N}}}\,\zeta^{-2(1+p-q)}}_{\text{Regime 1}}
\;+\;
\underbrace{\frac{3L_2^2\,\Delta_0}{2\beta^3}}_{\text{Regime 2}}
\;+\;
\underbrace{\frac{L_1}{\gamma}\log\frac{L_1^2\delta^2}{\varepsilon_g^2}
+ K^3_{\mathcal{N}}}_{\text{Regime 3}},
\end{equation}
with $r:=1+p-q$ and $K^3_{\mathcal{N}}$ given by
Theorem~\ref{thm:lyapunov}:
\[
K^3_{\mathcal{N}} \;=\;
\begin{cases}
\dfrac{L_1\kappa^2}{\sigma^2\gamma}
\log\dfrac{L_1^2\delta^2}{\varepsilon_g^2}
& \text{if } p = q, \\[12pt]
\dfrac{2L_1\kappa^2}{\sigma^2(2\gamma)^r(p-q)}
\!\left[\!\left(\dfrac{\varepsilon_g^2}{2L_1}\right)^{q-p}
\!-\!\left(\dfrac{L_1\delta^2}{2}\right)^{q-p}\right]
& \text{if } p > q, \\[12pt]
\left(\dfrac{L_1\delta^2}{2}\right)^{q-p}
\dfrac{2L_1\kappa^2}{\sigma^2(2\gamma)^r(q-p)}
& \text{if } p < q.
\end{cases}
\]
\end{theorem}
...
\begin{proof}
Sum the bounds from~\eqref{eq:regime1},~\eqref{eq:regime2},
\eqref{eq:K_R3}, and Theorem~\ref{thm:lyapunov}. Each regime bound
uses $\Delta_0$ independently, which is conservative: the true
budget satisfies $\Delta_0^1+\Delta_0^2+\Delta_0^3\leq\Delta_0$,
so the individual bounds could in principle be tightened if the
per-regime budgets were known a priori.
\end{proof}

\begin{remark}
The restarted terms $\zeta^{-2}$ in Regime~1 and
$\frac{L_1}{\gamma}\log\frac{1}{\varepsilon_g}$ in Regime~3
match the gradient descent baseline. The non-restarted Regime~3
term $K^3_{\mathcal{N}}$ is the novel contribution: for $p<q$ it
is independent of $\varepsilon_g$, so the overall bound improves
over gradient descent whenever the algorithm spends a significant
fraction of its iterations in the non-restarted Regime~3 phase.
\end{remark}
% ------------------- Analysis of staying in REGIME 3 (ONGOING) -------------------

\section{Regime 3 Analysis}
\label{sec:regime3stay}
